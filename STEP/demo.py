...
def main():
    ...
    if os.path.isdir('./datasets/demo/results/'):
        shutil.rmtree('./datasets/demo/results')
        os.makedirs('./datasets/demo/results/')
    ################## Customize your configuratons here ###################
    args = parse_config()
    checkpoint_path = 'pretrained/' + args.model_name
    ...
    dataloader = torch.utils.data.DataLoader(dataset, 1, num_workers=0,
    ...
    with torch.no_grad():
        for _, (images, tubes, infos) in enumerate(dataloader):
            ...
            for b in range(len(tubes_nums)):
                ...
                #print (info)
                # show progress bar
                loop.set_description("Evaluating frames..")
                loop.update(1)
    ...
    video = cv2.VideoWriter('./video_output/' + args.video_name, apiPreference=0, fourcc=fourcc, fps=25, frameSize=(width, height))
    ...
    for i in range(len(frame_arr)):
        ...
        # show progress bar
        loop2.set_description("Generating video..")
        loop2.update(1)
