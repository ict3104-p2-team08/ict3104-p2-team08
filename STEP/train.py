...
from step_models import BaseNet, ROINet, TwoBranchNet, ContextNet
from external.maskrcnn_benchmark.roi_layers import nms
from step_utils.utils import inference, train_select, AverageMeter, get_gpu_memory
from step_utils.tube_utils import flatten_tubes, valid_tubes
from step_utils.solver import WarmupCosineLR, WarmupStepLR, get_params
...
#    writer = SummaryWriter(args.save_root+"summary"+datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S'))
    ...
    while epochs < args.max_epochs:
        for _, (images, targets, tubes, infos) in enumerate(train_dataloader):
            ...
            if (iteration % args.save_step == 0) and iteration>0:
                ...
                save_name = args.save_root+ args.model_name + '_'+str(iteration) + '.pth'
                save_dict = {
                    'epochs': epochs+1,
                    'iteration': iteration,
                ...
                if os.path.isfile(args.save_root+ args.model_name + '_'+str(iteration-args.save_step) + '.pth'):
                    os.remove(args.save_root+ args.model_name + '_'+str(iteration-args.save_step) + '.pth')
                    #print (args.save_root+ args.model_name + '_'+str(iteration-args.save_step) + '.pth  removed!')
        ...
        if epochs in validate_epochs:
            ...
            all_metrics, data_for_csv = validate(args, val_dataloader, nets, iteration, iou_thresh=args.iou_thresh)
    ...
    #            writer.add_scalar(key, ap, iteration)
            ...
            if all_metrics[-1]['PascalBoxes_Precision/mAP@0.5IOU'] > best_mAP:
                ...
                save_name = './pretrained/' + args.model_name + ".pth"
                save_dict = {
                    'epochs': epochs+1,
                    'iteration': iteration,
                    'base_net': nets['base_net'].state_dict(),
                ...
                with open("./result/" + args.model_name + ".csv", "w", newline="") as file:
                    ...
                    for key, ap in all_metrics[-1].items():
                        writer.writerow([key, str(ap)])

                    # create header 2
                    header_2 = ["Trained on", "Tested on", "Prediction m-AP",
                                "Loss global cls", "Losses local loc", "Losses neighbor loc", "GPU usage", "Epoch"]
                    writer.writerow(header_2)

                    # write data for section 2
                    writer.writerow([len(train_dataloader), len(val_dataloader), all_metrics[-1]['PascalBoxes_Precision/mAP@0.5IOU'], losses_global_cls.avg, losses_local_loc.avg, losses_neighbor_loc.avg, gpu_memory[0]])

                    # write header/content for section 3
                    for row_item in data_for_csv:
...
def validate(args, val_dataloader, nets, iteration=0, iou_thresh=0.5):
    ...
    """
    data_csv_output = []
    data_csv_output.append(["Video name", "Frame id", "Box 1", "Box 2", "Box 3", "Box 4", "Label id", "Score"])

    # write results to files for evaluation
    output_files = []
    fouts = []
    for i in range(args.max_iter):
        output_file = args.save_root + args.model_name +'_val_result-'+str(iteration)+'-iter'+str(i+1)+'.csv'
                ...
                for b in range(len(tubes_nums)):
                    ...
                    for s,cl_ind,j in scores_list:
                        ...
                        fouts[i].write('{0},{1:04},{2:.4},{3:.4},{4:.4},{5:.4},{6},{7:.4}\n'.format(
                                                    ...
                                                    label_dict[cl_ind],
                                                    s))
                        data_csv_output.append([info['video_name'], info['fid'], box[0], box[1], box[2], box[3], label_dict[cl_ind], s])
    fout.close()
