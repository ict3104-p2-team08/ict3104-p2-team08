...
def val_file(models, num_epochs=50):
    ...
    for model, gpu, dataloader, optimizer, sched, model_file in models:
        ...
        for index in range(len(prob_val.get(args.videofile)[1])):
            ...
            for index1 in range(len(prob_val.get(args.videofile))):
        ...
        for model, gpu, dataloader, optimizer, sched, model_file in models:
            ...
            for index in range(len(prob_val.get('P18T15C03')[1])):
                ...
                if maxValue > 0 and (indexOfMaxValue == 36 or indexOfMaxValue == 37 or indexOfMaxValue == 38):
                    arrayForMaxAndIndex.append([index, indexOfMaxValue, maxValue, round((((index+1) * 0.64) / 60), 2)])
            #print("array for both max and index: ", arrayForMaxAndIndex)
...
def create_caption_video(arrayWithCaptions):
    ...
    array_for_csv = []
    csv_start_frame = 0
    csv_current_activity = arrayWithCaptions[0][0]
    annotated_csv = readCSV()
    annotated_current_position = 0
    while (True):
        ...
        #write caption from annotation csv
        if int(annotated_csv[annotated_current_position][1]) <= i <= int(annotated_csv[annotated_current_position][2]):
            caption1 = annotated_csv[annotated_current_position][0]
            cv2.putText(frame,
                        caption1,
                        (350, 75),
                        font, 0.5,
        ...
        if csv_current_activity != arrayWithCaptions[counter][0]:
            array_for_csv.append([csv_current_activity, csv_start_frame, i])
            csv_start_frame = i + 1
            csv_current_activity = arrayWithCaptions[counter][0]
...
def generateCSV(arrayForCSV):
    with open('output.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)

        # write the header
        writer.writerow(["event", "start_frame", "end_frame"])
